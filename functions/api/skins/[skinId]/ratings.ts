...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const stats = await DB.prepare(
    `SELECT ROUND(AVG(rating), 1) as avg_rating, COUNT(*) as rating_count
     FROM skin_ratings WHERE skin_id = ?`,
  ).bind(skin.id).first<{ avg_rating: number | null; rating_count: number }>();
  ...
  if (user) {
    const userRating = await DB.prepare(
      `SELECT rating FROM skin_ratings WHERE skin_id = ? AND user_id = ? LIMIT 1`,
    ).bind(skin.id, user.id).first<{ rating: number }>();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  await DB.prepare(
    `INSERT INTO skin_ratings (skin_id, user_id, rating)
     VALUES (?, ?, ?)
     ON CONFLICT (skin_id, user_id) DO UPDATE SET rating = excluded.rating, updated_at = datetime('now')`,
  ).bind(skin.id, user.id, rating).run();
